...
private:
    unsigned int currentSize{0}, capacity{0};
    T *arr{nullptr};
public:
...
// Default constructor, sets array capacity to the passed in parameter
template <typename T>
...
// Default destructor, deletes the pointer
template <typename T>
...
void CS2420Stack<T>::push(const T &value) {
    ...
    {
        cout << "ERROR: Try again with different values." << endl;
        return;
    }
...
// Returns the values at the currentSize of arr
template <typename T>
...
// Returns the current position in arr
template <typename T>
...
private:
    T *arr{nullptr};
    unsigned int front{0}, back{0}, currentSize{0}, capacity{0};
